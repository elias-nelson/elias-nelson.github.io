...
To ensure a smooth experience using {{< fa brands r-project >}} in VS Code, a few essential {{< fa brands r-project >}} packages must be installed before setting up the editor itself.
...
Most {{< fa brands r-project >}} packages are distributed through the [**Comprehensive** {{< fa brands r-project >}} **Archive Network** (**`CRAN`**)](https://cran.r-project.org/web/packages/available_packages_by_name.html), the official public repository for {{< fa brands r-project >}} packages. Although packages can also be installed from local files or other online sources, you do not need to manually visit the [**CRAN**](http://cran.r-project.org/) website to download them. Instead, packages can be installed directly from the {{< fa brands r-project >}} console using the **`install.packages()`** function.

Not every {{< fa brands r-project >}} package is hosted on [**CRAN**](http://cran.r-project.org/). Some are available from alternative platforms such as **GitHub** or **Bioconductor**, which provide additional or specialized tools.

To enable {{< fa brands r-project >}} support in VS Code, you will need to install at least one compulsory {{< fa brands r-project >}} package from **`CRAN`**. The key package required fr integration is **`languageserver`**.

Open {{< fa brands r-project >}} and run the following command in the Console:
...
This **`languageserver`** package allows VS Code to understand your {{< fa brands r-project >}} code. It provides important development features such as code completion, inline diagnostics, function documentation, and object inspection.

While not strictly required, you may also choose to install a few more optional packages that improve the overall {{< fa brands r-project >}} programming experience in VS Code. These packages are not required for {{< fa brands r-project >}} to function in VS Code, but they provide useful quality-of-life features when using {{< fa brands r-project >}} in VS Code.
...
The **`lintr`** package checks your {{< fa brands r-project >}} code for potential issues related to style inconsistencies, potential syntax problems, and violations of recommended best practices. When integrated with VS Code, these checks appear as you type, helping you build good coding habits early on.

::: column-margin
-   **`lintr`** is optional. {{< fa brands r-project >}} will work normally in VS Code without it.
...
When you run **`install.packages()`** for the first time, {{< fa brands r-project >}} may ask you to select a **`CRAN`** mirror.

![](utils/upscaled/cran_mirror.png){.shadowed fig-align="center"}

**`CRAN`**, short for the *Comprehensive R Archive Network*, is a global network of servers that store {{< fa brands r-project >}} packages. A CRAN mirror is simply a geographically hosted copy of this repository in different locations around the world.
...
When installing a package, {{< fa brands r-project >}} automatically installs any required dependencies. Seeing multiple packages install at once is normal and expected, so do not be alarmed if you see several additional packages being installed.

After selecting a **CRAN** mirror, during the installation process, you may see the prompt:

> *“Do you want to install from sources the packages which need compilation?”*

![](utils/upscaled/rpackage_compilation.png){.shadowed fig-align="center"}

This question relates to the two ways {{< fa brands r-project >}} packages are distributed and can be confusing for beginners, so let us break it down.
...
When you select **`NO`**, {{< fa brands r-project >}} installs pre-compiled binary versions automatically and skips source compilation.
...
On Windows and macOS, {{< fa brands r-project >}} usually just installs pre-compiled binary packages automatically, so unless tools like **`Rtools`** or **`Xcode`** are installed, this prompt may not appear at all. Do not be concerned if you never see it.

::: column-margin
-   **Beginner rule of thumb 🧠** If unsure, always choose **`NO`** when asked about source installation.
:::
...
![](utils/upscaled/packages_installed.png){.shadowed fig-align="center"}
...
::: column-margin
-   **`httpgd`** is an asynchronous HTTP server graphics device for {{< fa brands r-project >}} that enables live graphics to be viewed inside web browsers and IDEs
:::
...
![](utils/upscaled/90.%20httpgd_fail.png){.shadowed fig-align="center"}
...
-   It fails **CRAN** quality checks on newer {{< fa brands r-project >}} versions or operating systems.
...
**Why Build Tools Become Necessary** Many {{< fa brands r-project >}} packages often contain compiled code written in C, C++, or FORTRAN. To install such packages from source, your operating system must provide a compiler toolchain, commonly referred to as *build tools*.

**🖥️ Windows: Installing Rtools**

Windows users need to install **`Rtools`** following the steps as below:

1.  **Firstly check your** {{< fa brands r-project >}} **version**.
...
2.  **Download the matching `Rtools` version**

    -   Visit the CRAN **`Rtools`** page: <https://cran.r-project.org/bin/windows/Rtools/>

    -   Choose the installer that corresponds to your installed {{< fa brands r-project >}} version.

        ![](utils/upscaled/91.%20Rtools_page.png){.shadowed fig-align="center" width="90%"}

    -   Under **`installing Rtools4x`**, click the **`Rtools4x installer`** link to begin downloading the installer.

        ![](utils/upscaled/92.%20Rtools_downloadd.png){.shadowed fig-align="center"}

3.  **Run the installer**

    -   Accept defaults unless you need a custom installation path.

        ![](utils/upscaled/93.%20Rtools_path.png){.shadowed fig-align="center" width="90%"}
    ...
    -   Keep all the Additional Tasks options selected. The default configuration is recommended and requires no changes for a standard Rtools installation.
        ...
        ![](utils/upscaled/94.%20Rtools_additional.png){.shadowed fig-align="center" width="90%"}

    -   Click install.

        ![](utils/upscaled/95.%20Rtools_install.png){.shadowed fig-align="center" width="90%"}

    -   The installation process will proceed as below:

        ![](utils/upscaled/96.%20Rtools_install_process.png){.shadowed fig-align="center" width="90%"}

4.  **`Verify installation`**

    -   Open/restart {{< fa brands r-project >}} and run.
...
##### Making {{< iconify logos:r-lang >}} discoverable by {{< iconify logos:visual-studio-code >}} VS Code: Adding {{< iconify logos:r-lang >}} to **`PATH`**

For VS Code (and your system terminal) to run {{< fa brands r-project >}}, it needs to know where the {{< fa brands r-project >}} executable lives. This is handled through a system setting called **`PATH`**.
...
![](utils/upscaled/R_vanilla.png){.shadowed fig-align="center" width="90%"}
