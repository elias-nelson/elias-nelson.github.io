...
After installing {{< fa brands r-project >}}, you will need to install a few additional {{< fa brands r-project >}} packages from CRAN. These packages allow R to communicate effectively with VS Code and unlock many of the editor’s features.
...
To install packages from CRAN, you use the **`install.packages()`** function. For integrating {{< fa brands r-project >}} with VS Code, the following packages are particularly important.

Open your {{< fa brands r-project >}} console and run:

```{r}
#| eval: false
# 1. Language Server support
install.packages("languageserver") # <1>

# 2. Graphics device for VS Code
install.packages("httpgd") # <2>
  
# 3. Code linting
install.packages("lintr") # <3>
```
1. **`languageserver`** implements the Language Server Protocol for R and enables features such as code completion, diagnostics, and function documentation inside VS Code.

2. **`httpgd`** implements the Language Server Protocol for R and enables features such as code completion, diagnostics, and function documentation inside VS Code.

3. **`lintr`** analyzes your R code and highlights potential issues related to style, syntax, or best practices.

Together, these packages form the backbone of a productive and smooth {{< fa brands r-project >}} workflow in VS Code.

When you run **`install.packages()`** fr the first time, {{< fa brands r-project >}} may ask you to choose a **`CRAN`** mirror. **`CRAN`** (Comprehensive R Archive Network) is a global network of servers that store {{< fa brands r-project >}} packages.

::: column-margin
- A **`CRAN`** mirror is simply a copy of this hosed in different countries.
:::

You can safely select any mirror. You can select a mirror closest to your location otherwise the default **`0-Cloud / CDN (global)`** – https://cloud.r-project.org/ – this isn’t a single location but a content delivery network (CDN) that automatically serves packages from a server near you, often the best all-around choice even if a local mirror isn’t listed. It will serve packages from a nearby server without needing to manually pick a mirror. All mirrors host the same package content; choosing the closest one might slightly improve download speed, but it doesn’t change what you get.
...
On Windows and macOS, {{< fa brands r-project >}} usually installs pre-compiled binary packages automatically, so unless tools like **`Rtools`** or **`Xcode`** are installed, this prompt may not appear at all.
