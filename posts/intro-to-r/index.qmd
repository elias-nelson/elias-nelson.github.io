...
After a likely tedious installation of {{< fa brands r-project >}} and any IDE (RStudio or VSCode or Positron), as well as a somewhat detailed introduction to the different IDE options for {{< fa brands r-project >}} and how to operate their interfaces. We are now ready to do a little more coding and understand the basics of {{< fa brands r-project >}}. Here we will explore the basics of {{< fa brands r-project >}} which will build a precedence and the foundation for what everyone needs to run even more complex analyses in {{< fa brands r-prject >}}.

### Commands, Expressions, Assignments, Operators, and Objects

Previously after installing {{< fa brands r-project >}} we wrote our first "Hello, World!" program and also tried some simple arithmetic calculations as we would with a handheld/pocket calculator. We've also covered why it is important to organize your code into {{< fa brands r-project >}} scripts for reproducibility. Here we use most of what we've already learnt while learning more.

As discussed earlier the most basic computations you can perform in {{< fa brands r-project >}} are arithmetic operations. Let's take a look at a few more examples:
...
Most of these arithmetic operations are fairly straightforward except for the extraction of roots instead of the root symbol, to compute square root in {{< fa brands r-project >}}, we have to use the `sqrt()` function.

Now in your IDE for example `RStudio` you can enter all these operations in an {{< fa brands r-project >}} script and with your cursor click `Run` or simply use the keyboard shortcut `Ctrl + Enter`. This tells {{< fa brands r-project >}} to evaluate each operation and give us the output just as above in your Console.

One starkly observation you may notice is that after evaluation of every command above the printed output starts with `[1]`, which is simply the position of the adjacent element in the output—this will make some sense later.

For the above arithmetic commands, the result is printed to the screen and lost—there is no assignment involved. In order to do anything other than the simplest analyses , you must be able tore and recall data. In {{< fa brands r-project >}} you can assign the results of commands to symbolic variables (as in other programming languages) using the assignment operator `<-`. When a command is used for assignment, the result is no longer automatically printed to the `Console`.
...
::: column-margin
-   **Run** the object name **`x`** to print the output.
:::
...
::: column-margin
-   You can also use the **`print()`** function to get output.
:::
...
In this case, putting a space between the two characters that make up the assignment operator causes {{< fa brands r-project >}} to interpret the command as an expression that asks if x is less than zero. However spaces usually do not matter in {{< fa brands r-project >}}, as long as they do not separate a single operator or a variable name. This, for example, is fine:
...
In RStudio you can see the stored object `x` in the workspace panel (top right panel).

In VSCode you can see the object in the workspace as below

While in Positron similarly you can see the object as below

Alternatively, you can use the `ls()` function to also check what object(s) is stored in the workspace.

```{webr-r}
ls()
```

To remove an object from your workspace, use the `rm()` or `remove()` functions
...
::: column-margin
-   The **`remove()`** function can also be used to remove an object from the workspace.
:::

If you want to remove all the objects from the workspace you can use:
...
{{< fa brands r-project >}} is a case-sensitive language. This is true for symbolic variable names, function names, and everything else in {{< fa brands r-project >}}. For example if I create an object with lowercase x and want to print the output, referring to it in it's uppercase form will give an error:
...
The same applies to functions. I cannot use the `print()` function in uppercase

```{webr-r}
PRINT(x)
```

In {{< fa brands r-project >}}, commands can be separated by moving onto a new line (i.e., hitting ennter) or by typing a semicolon `";"`, which can be handy in scripts for codensing code. If a command is not completed in one line (by design or error), the typical prompt `>` symbol in the Console is replaced with a +.

There are several operators that are used in the {{< fa brands r-project >}} language. Below are tables for some of the most common.
