...
6.  Many people prefer using the **`radian`** as an alternative Console for {{< fa brands r-project >}}

    Now as a final step to using the **`radian`** Console in VS Code we need to configure VS Code to use radian by setting th path to the **`radian`** executable in your **`setting.json`**. On a Mac, this is typically **`/usr/local/bin/radian`**. You can access settings.json by pressing **`⌘ + Shift + P`** on Mac or **`Ctrl + Shift + P`** on Windows/Linux, typing **`“Open Settings (JSON)”`**, and pressing **`Enter↵`**.

    For Windows since radian is installed via pip, its executable is in your Python Scripts folder. For a setup like this it is likely:

    ``` cmd
    C:\Users\Entomology Lab\AppData\Local\Programs\Python314\Scripts
    ```
